...
def _build_parser() -> argparse.ArgumentParser:
    ...
    _add_coarse_folding_options(score)
    ...
    scoring_validate.add_argument(
        ...
        choices=["protein-heavy", "all-heavy", "full"],
        default=DEFAULT_TRAINING_DOMAIN,
        help="Atoms/residues to validate for scoring. Default: protein-heavy.",
    )
    scoring_validate.add_argument(
        "--prepare",
        choices=PREPARE_MODES,
    ...
    _add_coarse_folding_options(scoring_validate)
...
def _add_coarse_folding_options(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--decoded-torsions",
        help="Path to a JSON object with decoded torsion angles in radians, keyed like 0_phi or 3_chi1.",
    )
    parser.add_argument(
        "--disable-end-to-end-constraint",
        action="store_true",
        help="Disable the coarse folding end-to-end compactness restraint.",
    )
    parser.add_argument(
        "--end-to-end-scale",
        type=float,
        default=1.0,
        help="Additional scale multiplier for the coarse folding end-to-end restraint. Default: 1.0.",
    )
...
def _add_gromacs_options(
    ...
    *,
    ...
    parser.add_argument(
        "--gromacs-preflight",
        choices=GROMACS_PREFLIGHT_MODES,
        default="strict",
        help="Physical-integrity preflight mode before GROMACS execution. Default: strict.",
    )
    parser.add_argument(
        "--gromacs-work-dir",
...
def _cmd_score(args: argparse.Namespace) -> int:
    ...
    if args.profiles:
        ...
        if decoded_torsions is not None or args.disable_end_to_end_constraint or args.end_to_end_scale != 1.0:
            raise ValueError("coarse folding options cannot be combined with --profiles")
    ...
    result = score_structure(
        ...
        gromacs_preflight=args.gromacs_preflight,
...
def _cmd_scoring_validate_options(args: argparse.Namespace) -> int:
    normalized_model = str(args.model or "").strip().lower().replace("_", "-")
    uses_coarse_options = bool(args.decoded_torsions) or args.disable_end_to_end_constraint or args.end_to_end_scale != 1.0
    if normalized_model in {"ambertools-sander", "gromacs-mdrun"} and not uses_coarse_options:
        payload = _validate_external_options_payload(args, model=args.model)
    else:
        options: dict[str, Any] = {"domain": args.domain}
        decoded_torsions = _load_json_mapping(args.decoded_torsions, label="--decoded-torsions")
...
def _validate_external_options_payload(args: argparse.Namespace, *, model: str) -> dict[str, Any]:
    return validate_external_scoring_options(
        ...
        gromacs_preflight=getattr(args, "gromacs_preflight", "strict"),
        gromacs_run_settings=_gromacs_run_settings_from_args(args),
        external_timeout_seconds=args.external_timeout,
        prep_cache_dir=args.prep_cache_dir,
        prep_cache_mode=args.prep_cache_mode,
...
def _load_json_mapping(path: Optional[str], *, label: str) -> Optional[dict[str, Any]]:
    if not path:
        return None
