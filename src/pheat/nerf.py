...
try:  # Optional QTF-only helpers used by the archived optimization harness.
...
except ImportError:  # Keep the NERF builder usable as a standalone PHEAT module.
    LandscapeTracker = Any  # type: ignore[misc,assignment]
    qtf_gromacs = None  # type: ignore[assignment]
    def _statevector_data(_circuit):
        raise RuntimeError("statevector optimization helpers are provided by QTF")
...
class NerfFolder:
    ...
    def __init__(
        ...
        if self.stage_backend not in ("custom", "openmm"):
            raise ValueError("energy_backend must be 'custom' or 'openmm'")
    ...
    def _final_output_structure_from_params(self, params):
        """
        Return the final structure for fold().
        """
    ...
    def _score_stage_openmm(self, angle_vec, return_terms=False):
        terms = {"energy_backend_openmm": 1.0, "energy_backend_custom": 0.0}
        try:
            self._ensure_openmm()
    ...
    def _initialize_topology_cache(self):
        ...
        self.static_bonds = list(static_bonds)
